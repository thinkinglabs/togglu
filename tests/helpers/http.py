...
class WorkspacesRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        with open(os.path.join(THIS_DIR, os.pardir, 'workspaces.json'), 'r') as myfile:
...
class DetailedReportPaginationRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        with open(os.path.join(THIS_DIR, os.pardir, 'detailed_report_page1.json'), 'r') as myfile:
            data1 = myfile.read().replace('\n', '')
        with open(os.path.join(THIS_DIR, os.pardir, 'detailed_report_page2.json'), 'r') as myfile:
            data2 = myfile.read().replace('\n', '')
        with open(os.path.join(THIS_DIR, os.pardir, 'detailed_report_page3.json'), 'r') as myfile:
...
class DetailedReportFilterRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        with open(os.path.join(THIS_DIR, os.pardir, 'detailed_report_filter.json'), 'r') as myfile:
            data = myfile.read().replace('\n', '')
